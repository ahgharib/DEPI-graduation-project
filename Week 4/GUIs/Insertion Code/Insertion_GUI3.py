...
# To open a new window for customer
...
# To open a new window for store
...
# To open a new window for product
...
# To open a new window for orders
...
# To open a new window for feedback
def open_insert_feedback():
    ...
    submit_button = tk.Button(feedback_window, text="Insert Feedback", state="disabled")
    submit_button.pack()
    ...
    def update_submit_button_state():
        ...
        selected_order_id = order_id_combo.get()
        
        if customer_id.isdigit() and selected_store_address and selected_order_id:
    ...
    def update_order_ids(event):
        ...
        if store_id:
            orders = get_orders_without_feedback_for_customer_and_store(conn, customer_id, store_id)
            if orders:
                order_id_combo['values'] = [order[0] for order in orders]
                order_id_combo.current(0)

                order_id_combo.bind("<<ComboboxSelected>>", update_submit_button_state)
            else:
                order_id_combo.set("No orders available")
                submit_button.config(state="disabled")
        else:
            order_id_combo.set("Select a store first")
            submit_button.config(state="disabled")
...
# Checking if feedback exists for a given order ID
...
def get_orders_without_feedback_for_customer_and_store(conn, customer_id, store_id):
    cursor = conn.cursor()
    query = """
    SELECT o.id
    FROM Orders o
    LEFT JOIN Feedback f ON o.id = f.order_id
    WHERE o.customer_id = ? AND o.store_id = ? AND f.id IS NULL;
    """
    cursor.execute(query, (customer_id, store_id))
    return cursor.fetchall()

# Rest of the GUI components for the main window
